...
pipeline4 = pickle.load(open("pipeline4.pkl", "rb"))
...
if text.strip():
    ...
    if st.button("🔍 Predict Domain"):
        with st.spinner("Analyzing your resume..."):
            ...
            pred2 = pipeline2.predict([processed_text])[0]
            pred3 = pipeline3.predict([processed_text])[0]
            pred4 = pipeline4.predict([processed_text])[0]

            decoded1 = label_encoder.inverse_transform([pred1])[0]
            decoded2 = label_encoder.inverse_transform([pred2])[0]
            decoded3 = label_encoder.inverse_transform([pred3])[0]
            decoded4 = label_encoder.inverse_transform([pred4])[0]
        ...
        # Display predictions
        st.markdown(f"<div class='pred-card'><strong>🔷 SVM Prediction:</strong> {decoded1}</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='pred-card'><strong>🌲 Random Forest Prediction:</strong> {decoded2}</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='pred-card'><strong>⚡ XGBoost Prediction:</strong> {decoded3}</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='pred-card'><strong>🌿 LightGBM Prediction:</strong> {decoded4}</div>", unsafe_allow_html=True)
